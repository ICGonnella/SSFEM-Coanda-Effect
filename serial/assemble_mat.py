...
def main():
    parser = argparse.ArgumentParser(description="Assemble material parameters")
    parser.add_argument("--viscosity_mean", type=float, required=True)
    parser.add_argument("--viscosity_var", type=float, required=True)
    parser.add_argument("--npc", type=int, required=True)
    args = parser.parse_args()

    viscosity_mean = args.viscosity_mean
    viscosity_var  = args.viscosity_var
    npc            = args.npc

    xi = cp.Uniform(-1.73205167,1.73205167)
    deg = npc-1
    _, stochastic_bases_norm = cp.generate_expansion(deg, xi, retall=True)
    tmp = Stiffness_probamat_assembly( xi, deg, 1)
    mat0 = (viscosity_mean) * tmp.assemble_matrix(2,a = [xi.lower[0]],b = [xi.upper[0]]) + np.sqrt(viscosity_var) * tmp.assemble_matrix(2,func = [lambda x: x],a = [xi.lower[0]],b = [xi.upper[0]])
    mat1 = tmp.assemble_matrix(2,a = [xi.lower[0]],b = [xi.upper[0]])
    mat2_list = [tmp.assemble_matrix(3,a = [xi.lower[0]],b = [xi.upper[0]])[i] for i in range(npc)]
    np.savetxt(f"build/M_0.csv", mat0, delimiter=",")
    np.savetxt(f"build/M_1.csv", mat1, delimiter=",")
    for i in range(2,2+npc):
        np.savetxt(f"build/M_{i}.csv", mat2_list[i-2], delimiter=",")
    np.savetxt("build/norms.csv", stochastic_bases_norm, delimiter=",")
